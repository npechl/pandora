...
`record_original_assembly_mapping` (default `True`) — populate `AssemblyMapping` for every assembly, including `chain_copies` (which new chain came from which source chain + operator) when `standardize_biological_assembly` generated any.

Chains generated by symmetry expansion are also propagated into `Structure.connections` and `Structure.secondary_structure`: a bond or secondary-structure span is duplicated onto a generated chain whenever *every* chain it references was copied under the same symmetry operator (trivially true for a span/bond within a single chain, since a rigid-body copy preserves internal distances and residue ranges exactly). A connection between two chains copied under *different* operators — or where only one side has a generated copy — has no single valid rigid-body remap and is left unduplicated, rather than fabricating a bond that may not exist in the assembled complex.

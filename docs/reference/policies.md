...
!!! note "Known limitation"
    Chains generated by symmetry expansion are not reflected in `Structure.connections` or `Structure.secondary_structure` — those still reference only the original asymmetric-unit chain ids.

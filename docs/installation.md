...
| `pip install -e .` | **cli** — the `pandora` command, exposing every stage below as a subcommand (`fetch`, `canonicalise`, `curate`, `dedup`, `similarity`, `cluster`, `partition`, `annotate`, `manifest`, `reproduce`, `export`) | nothing to install the CLI itself — `fetch`/`reproduce` need the `ingestion` extra, `similarity` needs `mmseqs2`/`foldseek` on `PATH` |

...
A structure that fails a rule comes back excluded, with a machine-readable
reason:
...
`content_rules` (waters/ions/other ligands) never excludes — it filters
the retained structure's atoms in place, the same way
...
missing data (e.g. null resolution) then apply their configured default
...
[Export](export.md) for writing them to disk.
